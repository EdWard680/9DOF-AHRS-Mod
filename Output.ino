...
{    
      printf("!");
      #if PRINT_EULER == 1
      char rollStr[16], pitchStr[16], yawStr[16];
      dtostrf(ToDeg(roll), 4, 3, rollStr);
      dtostrf(ToDeg(pitch), 4, 3, pitchStr);
      dtostrf(ToDeg(yaw), 4, 3, yawStr);
      printf("ANG:%s,%s,%s,", rollStr, pitchStr, yawStr);      
      #endif      
      #if PRINT_ACCEL == 1
